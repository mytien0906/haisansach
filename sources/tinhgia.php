<?php  
	...
	/* Lấy bài viết tĩnh */
	$dongia = $d->rawQueryOne("select * from #_static where type = ? limit 0,1",array('dongia'));
	$quytrinh = $d->rawQueryOne("select * from #_static where type = ? limit 0,1",array('quytrinh'));
	$vattu = $d->rawQueryOne("select * from #_static where type = ? limit 0,1",array('vattu'));

	$seopage = $d->rawQueryOne("select * from #_seopage where type = ? limit 0,1",array('tinh-gia-xay-dung'));
